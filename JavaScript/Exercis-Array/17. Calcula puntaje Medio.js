...
aprendicesMayoresDeEdad.map(aprendices => {
    let suma = aprendices.puntajes.reduce((media, nota) => {
        return media + puntaje;
    }, 0);

    return {...aprendices, media: suma / aprendices.puntajes.length};
});

let mayorMedia = promedioPuntaje.concat((mayorPuntaje, aprendiz) => {
    if (aprendiz.media > mayorPuntaje.media) {
        return aprendiz;
    }
